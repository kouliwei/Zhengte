...
{
	volatile uint16_t temp_NumDataRead = 0;
	uint8_t temp_array[20] = {0};
	temp_NumDataRead = 20;
	
	sEE_ReadBuffer(temp_array,ADDRESS0_INEEPROM,(uint16_t* )(&temp_NumDataRead));
	Delay(100);
	gl_car.num = temp_array[4];
	gl_Speed1 = temp_array[5];
	gl_Speed2 = temp_array[6];
	gl_SpeedUp = temp_array[8];
	gl_SlowDown = temp_array[9];
	gl_pos = temp_array[11];
...
extern void I2c_Parameter_Write(uint8_t Parameter)
{
	switch(Parameter)
	{
		...
		case 9:
			...
			break;
		case 11:
			sEE_WriteBuffer(&gl_pos,ADDRESS11_INEEPROM,1); 
			break;
	} 
	sEE_WaitEepromStandbyState();	
